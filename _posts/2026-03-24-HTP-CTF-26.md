...
I didn't think too much into it though and was running out of time to fully flesh out the idea, maybe one day in the future though I will get something woking because I think it could be fun.
...
I didn't really get close to what this challenge was but it is what gave me the original idea for creating this challenge and was one of this first ones I thought of.
...
__attribute__((used))
void h4ckthepl4net() {
    unsigned char cipher_decoded[] = {
        74,88,71,71,84,81,123,100,109,100,101,95,106,99,99,107,95,119,115,114,95,121,121,101,125
    };

    char key[] = "cerealkiller";
    int key_len = strlen(key);
    int len = sizeof(cipher_decoded)/sizeof(cipher_decoded[0]);
    ...
    for (int i = 0; i < len; i++) {
        ...
        char c = cipher_decoded[i];

        if (c >= 'A' && c <= 'Z') {
            int shift = key[key_index % key_len] - 'a';
            flag[i] = ((c - 'A' - shift + 26) % 26) + 'A';
            key_index++;
        }
        else if (c >= 'a' && c <= 'z') {
            int shift = key[key_index % key_len] - 'a';
...
void hackth3plan3t() {
    ...
    if (h4cKth3p14n3t == 1) {
        ...
        printf("\t[encrypted flag hidden]\n");
        printf("Due to the breach though we can no longer decrypt it when output or output it encrypted\n\n");
        hackth3plan3t();
    }
...
I'm grateful to have gotten the chance to work with some of these people again on this event, I always enjoy talking about the challenge with them.
